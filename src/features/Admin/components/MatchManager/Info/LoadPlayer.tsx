...
const LoadingPlayerMatch: React.FC<LoadingPlayerMatchProps> = ({ match }) => {
  ...
  const renderPlayerRows = (players: Player[], role: string, teamName: string) => {
    const filteredPlayers = players.filter(
      (player) => player.role === role && player.team.name === teamName,
    );
    ...
    <>
        ...
        >
          ...
          <DialogContent>
            ...
            <Box sx={{ mb: 2 }}>
              ...
              <Select fullWidth value={team} onChange={(e) => setTeam(e.target.value)}>
      ...
      <Box sx={{ backgroundColor: "white", borderRadius: 2, my: 2, p: 2 }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <Typography sx={{ fontWeight: 700, fontSize: "1.7rem", color: "#37003c" }}>
            Players
          </Typography>
          <Button
            variant="outlined"
